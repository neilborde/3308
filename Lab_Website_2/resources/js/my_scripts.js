...
*/
const viewStudentStats = (id, toggle)=>{
...
function  loadStatsPage(){
	...
	var wins = 0;
	var losses = 0;
	for (var i = 2, row; row = stats.rows[i]; i++){
		var home = row.cells[2].innerText;
		var away = row.cells[3].innerText;
		if(home > away){
			row.cells[4].innerText = "Buffs";
			wins++;
		}
		else{
			row.cells[4].innerText = row.cells[1].innerText;
			losses++;
		}
	}
	let winloss = document.getElementById('wins');
	winloss.innerText = wins;
	let winloss2 = document.getElementById('losses');
	winloss2.innerText = losses;
...
function loadPlayersPage(){
	...
	for (player in players){
		...
		const p = player;
		// var x = document.createTextNode(player.name);
		// console.log(x);
		anchor.innerHTML = players[player].name;
		anchor.title = "PLAYER";
		anchor.href = "#";
		anchor.onclick = () => switchPlayers(p);
